...
class SLICE_OT(bpy.types.Operator):
    ...
    def makeSlicers(self, srcObject):
        ...
        #LEFT
        s = max(mathutils.Vector((srcObject.dimensions.x / srcObject.scale.x, srcObject.dimensions.y / srcObject.scale.y, srcObject.dimensions.z / srcObject.scale.z)))
        container = SlicerContainer()
        bpy.ops.mesh.primitive_cube_add(size=s, location= srcObject.location)
        ob = bpy.context.active_object
        ob.dimensions.x = srcObject.dimensions.x / srcObject.scale.x
        ob.dimensions.y = srcObject.dimensions.y / srcObject.scale.y
        ob.parent = srcObject
        ob.location = (0,0,s/2.0)
        container.left = ob
        #RIGHT
        bpy.ops.mesh.primitive_cube_add(size=s, location= srcObject.location)
        ob = bpy.context.active_object
